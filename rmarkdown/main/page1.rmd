---
title: "DRAFT::: Summary statistics"
output:
  html_document:
    toc: true
    toc_depth: 2
...
if(!exists("smart_survey_data")) stop("Dataframe called smart_survey_data not available. This should be in the 
                                      function enviroment of render_main_site. Check that this is available in this
                                      enviroment.")

# Create tables
tables <- carsurvey2::generate_tables(smart_survey_data)

samples <- list(
  all = nrow(smart_survey_data),
  coders = sum(smart_survey_data$code_freq != "Never"),
  heard_of_rap = sum(smart_survey_data$RAP_heard_of == "Yes")
)
```
...
```{r echo = FALSE}
kableExtra::kable_styling(carsurvey2::output_likert_table(tables$code_prac_chart)) %>% 
  kableExtra::add_footnote(paste0("Sample size = ", samples$coders))
